...
export default function CommunicationComposer({ sessionId, allCount, twoDayCount, threeDayCount }: ComposerProps) {
  ...
  return <div className="communication-builder">
    ...
    <aside className="communication-preview-wrap"><span className="eyebrow">Live preview</span><p className="personalization-note">Each email automatically uses that member’s first name.</p><div className="email-subject-preview"><strong>{subject || "Your subject"}</strong><span>{preheader || "Preview text appears here."}</span></div><div className="email-preview"><div className="email-preview-head"><span>Holden Health</span><h2>{heading || "Your heading"}</h2><p>Stronger together. Better every day.</p></div><div className="email-preview-body"><strong>Hi [member’s first name],</strong>{body.split(/\n\s*\n/).filter(Boolean).map((paragraph, index) => <p key={index}>{paragraph}</p>)}{ctaLabel && ctaUrl ? <span className="email-preview-button">{ctaLabel}</span> : null}<p>You’ve got this,<br /><strong>Kelsey</strong></p></div><div className="email-preview-foot"><strong>Holden Health LLC</strong><br />Plymouth, Michigan</div></div></aside>
  </div>;
}
