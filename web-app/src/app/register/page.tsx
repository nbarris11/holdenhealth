...
export default async function RegisterPage({ searchParams }: RegisterPageProps) {
  ...
  const { data: session } = await supabase.from("sessions").select("id,name,description,starts_on,ends_on,new_member_price_cents,returning_member_price_cents,status").eq("published", true).in("status", ["enrolling", "active"]).order("starts_on").limit(1).maybeSingle();
    ...
    <main className="auth-shell register-shell">
      <section className="auth-card register-card">
        ...
        {sent ? <><span className="eyebrow">Request received</span><h1>You’re on Kelsey’s list.</h1><p className="lede">Kelsey will reach out to make sure the session fits what you need. After approval, you’ll receive a secure link to your member portal.</p><div className="button-row"><Link className="button secondary" href="https://holden.health">Return to Holden Health</Link></div></> : session ? <>
          ...
          <div className="registration-session"><strong>{session.name}</strong><span>New members ${(session.new_member_price_cents / 100).toFixed(0)} · Returning members ${((session.returning_member_price_cents ?? session.new_member_price_cents) / 100).toFixed(0)}</span><p>{session.description}</p></div>
          <form className="stack-form" action={submitRegistrationRequest}>
            ...
            <label className="check-label"><input name="returningMember" type="checkbox" /> I’ve completed a Holden Health paid session before</label>
